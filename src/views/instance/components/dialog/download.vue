<template>
  <v-dialog id="download" v-model="visible" max-width="500" @click:outside="setVisibleDialogName('')"
    content-class="download-dialog">
    <v-card>
      <v-card-title>
      ...
      <v-card-text>
        <div class="download d-flex">
          <div class="single d-flex flex-clo flex-1 borbox" @click="type='single'" :class="type==='single'?'active':''">
          ...
          <div class="multiple d-flex flex-clo flex-1 borbox" @click="type='multiple'"
            :class="type==='multiple'?'active':''">
        ...
        <div class="pre-compile d-flex flex-clo">
          <v-checkbox dense hide-details v-model="preCompile" label="下载编译前的文件"></v-checkbox>
          <span class="text-describe">如果你使用了预处理语言，选中此选项会下载未编译前的文件。</span>
        </div>
        <v-btn color="info" @click="downloadFiles" block class="dwn-btn">下载</v-btn>
      </v-card-text>
    </v-card>
  </v-dialog>
</template>
